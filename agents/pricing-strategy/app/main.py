...
async def lifespan(app: FastAPI):
    logging.basicConfig(
        level=logging.INFO,
        format="%(asctime)s | pricing-strategy | %(levelname)s | %(name)s | %(message)s",
    )
    ...
    app.state.llm_client = AsyncOpenAI(
        ...
        api_key="sk-litellm-dev-key",  # LiteLLM master key
    )
    ...
    logger.info("Pricing Strategy Agent started (model=%s, litellm=%s)", model, litellm_url)
    yield

    await app.state.tool_executor.close()


app = FastAPI(title="Pricing Strategy Agent", lifespan=lifespan)
app.include_router(router)


@app.get("/health")
async def health():
    return {"status": "ok", "service": "pricing-strategy"}
